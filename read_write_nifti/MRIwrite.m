...
switch(fmt)
 ...
 case {'nii','nii.gz'} %----------- NIFTI! ------------%
  hdr.data_type       = mri.niftihdr.data_type;
  hdr.db_name         = mri.niftihdr.db_name;
  hdr.extents         = 0;
  hdr.session_error   = 0;
  hdr.regular         = mri.niftihdr.regular;
  hdr.dim_info        = mri.niftihdr.dim_info;
  
  % Note that the order is 2 1 3 4
%   hdr.dim = [mri.volsize(2) mri.volsize(1) mri.volsize(3)  mri.nframes];
  hdr.dim = mri.niftihdr.dim;
  hdr.intent_p1       = 0;
  hdr.intent_p2       = 0;
  hdr.intent_p3       = 0;
  hdr.intent_code     = 0;
  ...
  hdr.datatype=mri.niftihdr.datatype;
  hdr.bitpix = mri.niftihdr.bitpix;
  ...
  hdr.cal_max         = mri.niftihdr.cal_max;
  hdr.cal_min         = mri.niftihdr.cal_min;
  hdr.slice_duration  = 0;
  hdr.toffset         = 0;
  hdr.glmax           = 0;
  hdr.glmin           = 0;
  hdr.descrip         = mri.niftihdr.descrip;
  hdr.aux_file        = mri.niftihdr.aux_file;
  hdr.qform_code      = mri.niftihdr.qform_code; % 1=NIFTI_XFORM_SCANNER_ANAT
  hdr.sform_code      = mri.niftihdr.sform_code; % 1=NIFTI_XFORM_SCANNER_ANAT
  ...
  hdr.intent_name     = mri.niftihdr.intent_name;
  hdr.magic           = 'n+1';
